<?php
/*
 * usage : php lance_import.php -f=fichier_tei.xml -u=HALLogin -p=HALPassword >res_import_interstices.txt
 * Installation :
 * 	Copier dans le même répertoire : lance_import.php, config.php
 *  Modifier config.php et choisir API prod ou preprod
 * 	Créer un sous répertoire : xmlSword dans le lequel seront copiés tous les xml utilisés pour l'import Sword
 * Action :
 *  découpe la TEI en un fichier par biblFull, copie du fichier dans xmlSword et lancement import Sword
 */ 
...
{
	exit("Il manque un argument\nUsage : php lance_import.php -f=fichier_tei.xml -u=HALLogin -p=HALPassword\n");
}
$fileName=$options['f'];
$HAL_USER=$options['u'];
...
//$xpathReq = createXpathTEI($domInterstTei);
//$entries = $xpathReq->query("/tei:TEI/tei:text/tei:body/tei:listBibl/tei:biblFull");
...
if ($biblFulls->length >= 1) {
	...
	foreach($biblFulls as $biblFull)
	{
		...
		//A FAIRE user à chercher dans CAS à partir de $user
